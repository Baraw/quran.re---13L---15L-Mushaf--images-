...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (idMenu !== null && idMenu < navigationLinks.length) {
        ...
        navigationLinks[idMenu+maxOnglet].classList.add('active'); //+ pour recommencer à 0 et ajouter aussi sur le menu déroulant mobile
        navigationLinks[idMenu].setAttribute('href', '#');
        navigationLinks[idMenu+maxOnglet].setAttribute('href', '#'); //+ pour recommencer à 0 et ajouter aussi sur le menu déroulant mobile
    }

    // On cible la div qui contient les images du Coran dans ton ancien code
    const zoneMoushaf = document.getElementById('moushaf') || document.getElementById('moushafdouble');
    
    if (!zoneMoushaf) return;

    let startX = 0;
    let startY = 0;

    // Quand le doigt touche l'écran
    zoneMoushaf.addEventListener('touchstart', (e) => {
        startX = e.changedTouches[0].screenX;
        startY = e.changedTouches[0].screenY;
    }, { passive: true });

    // Quand le doigt quitte l'écran
    zoneMoushaf.addEventListener('touchend', (e) => {
        const deltaX = e.changedTouches[0].screenX - startX;
        const deltaY = e.changedTouches[0].screenY - startY;

        // Vérification : mouvement d'au moins 50px et majoritairement horizontal
        if (Math.abs(deltaX) > 50 && Math.abs(deltaX) > Math.abs(deltaY)) {
            
            if (deltaX > 0) {
                // Swipe vers la droite ➔ on clique sur ton bouton précédent (sens arabe)
                const btnPrec = document.getElementById('boutonPRECEDENT');
                if (btnPrec) btnPrec.click();
            } else {
                // Swipe vers la gauche ➔ on clique sur ton bouton suivant
                const btnSuiv = document.getElementById('boutonSUIVANT');
                if (btnSuiv) btnSuiv.click();
            }
        }
    }, { passive: true });
