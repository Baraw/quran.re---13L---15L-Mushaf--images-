...
function recupURLversImage(moushafType,formatPage)
{
	...
	const page = monLienParam.get('page');  //recherche la valeur de la page dans le lien (=XX)
	 ...
	 if ( page != null  )
     {
        ...
        if (moushafType === '15ltadjwid') // 15l TADJWID
        {
				var pageMax = 851;
				var pageMin = 2;
        }
		
		if ( (moushafType === '15lmadinah_indopak') || (moushafType === '15lvert') ) // 15l SIMPLE
		{
				var pageMax = 621;
				var pageMin = 1;
		}
...
function affichagePage(typeSelect,valeurPage,moushafType,formatPage,numeroVerset) 
{
	valeurPageReelle = Number(valeurPage);
	...
	var repMoushaf = "images/quran"+ moushafType +"/quran"+ moushafType +" (";  // Nom du moushafType  équivaut au nom du dossier images et des noms des images avec l'ajout de quran avant 
		...
		// MAJ du PLACEHOLDER de numverset
		if( typeSelect == 'sourate' ) //SOURATE
		{
				// alert('numero sourate = '+numeroVerset);
				document.getElementById("numVERSET").setAttribute("placeholder","1 à "+QuranData.Sura[numeroVerset][1]); // change la valeur max du PLACEHOLDER du input par le nombre de ayah de la sourah
				
		}
		
		
		
	 if ( typeSelect === 'verset' )  // AFFICHAGE DE LA PAGE SELON LE VERSET CHOISI -- numeroVerset ==> le numero de surate ici
		{
			valeurPageReelle = (searchPage(  Number(numeroVerset),  Number(valeurPage) ))+1 ; // ici, moushafType = le numero de surate
			// alert( `Sourah n° ${numeroVerset} -  Verset ${valeurPage} `);
			document.getElementById("numVERSET").setAttribute("max",QuranData.Sura[numeroVerset][1]); // change la valeur max du input par le nombre de ayah de la sourah
		}
		
	 // 15l TADJWID
	if (moushafType === '15ltadjwid')
	{
		// https://practicequran.com/views/al_quran_15_lines_color_coded_tajweedi/pages/1/20.jpg 
		
		repMoushaf = 'https://practicequran.com/views/al_quran_15_lines_color_coded_tajweedi/pages/'+determinePart(valeurPageReelle)+'/';
			...
			if( formatPage === '2pages' )
			{
				...
				{	//paire (page de droite)
				   ...
				   var repImageCachee = repMoushaf+ ((valeurPageReelle)+2) +".jpg";// mise en cache
				   var repImageCachee2 = repMoushaf+ ((valeurPageReelle2)+2) +".jpg";// mise en cache
			}
			else
				var repImageCachee = repMoushaf+ ((valeurPageReelle)+1) +".jpg";// mise en cache
	...
	//15l  MADINAH INDOPAK
	else if (moushafType === '15lmadinah_indopak') 
	{
		...
		// valeurPageReelle = (valeurPageReelle)-1;
			...
			if( formatPage === '2pages' )
			{
				...
				var repImageCachee = repMoushaf+ ((valeurPageReelle)+2) +").png";// mise en cache
			   var repImageCachee2 = repMoushaf+ ((valeurPageReelle2)+2) +").png";// mise en cache   
			}
			else
				var repImageCachee = repMoushaf+ ((valeurPageReelle)+1) +").png";// mise en cache
	...
	/*   if( typeSelect == "swipe") //  TEST DU MODULE SWIPE
	   {
		...
		if ( valeurPageReelle == 1 ) // page de DROITE (ici valeurPageReelle = index du swipe)
		{
				...
				// newImg.src ='images/quran15lmadinah_ip/quran15lmadinah_indopak ('+ ((leALT)+2) +').jpg';
			   // newImg.alt = leALT+2;
	...
	{			
		...
		//15l  MADINAH
	else if (moushafType === '15lmadinah') 
	{			
	...
	//15l  DIGITAL
	else if (moushafType === '15ldigital') 
	{			
			if( formatPage === '2pages' )
			{
					if (valeurPageReelle & 1) //bitwise and logic
	...
	document.getElementById("pageCachee").setAttribute("src",repImageCachee);  // image en cache
	 ...
	 document.getElementById("selectPAGE").setAttribute("value",valeurPageReelle); // valeur de "ALLER A LA PAGE"
	 ...
	 if ( formatPage === '2pages' )
	 {
	  ...
	  // Changement du lien
	  const newLien = '?page='+(valeurPageReelle);
...
function ChangementPage(valeurChangement,moushafType,formatPage) 
{
	var nouvPage = Number(document.getElementById("pageAffichee").alt); // Récupère le numéro de page actuel inclus dans le ALT de l'image et le transforme en numérique pour pouvoir incrémenter ou décrémenter
		...
		{
			if ( (((moushafType === '15lmadinah_indopak') || (moushafType === '15lmadinah')) && (nouvPage != 611))
				 ||  ( (moushafType === '15ltadjwid') || (moushafType === '15lvert') || (moushafType === '15ldigital') )  && (nouvPage != 621)  ) // si pas la derniere page
				{
					if (formatPage === '2pages')
							nouvPage = nouvPage+2;
					else
		...
		{
			if( ( (moushafType === '15ltadjwid') || (moushafType === '15lmadinah_indopak')  || (moushafType === '15lmadinah') || (moushafType === '15lvert') || (moushafType === '15ldigital') ) && (nouvPage != 2)  ) // si pas la premiere page
			{
				if (formatPage === '2pages')
					nouvPage = nouvPage-2;
				else
...
// RECHERCHE AYAH MAX DE SOURATE
function nombreVerset(sourate)
{
		return sourate ;
...
// PARTIE DE CODE DE ABOU : Affiche la page selon le ayah
	...
	// A PARTIR D'ICI, CHANGEMENT SELON LES MOUSHAF, ICI MOUSHAF ECRITURE INDOPAK 15L
	[67, 11], 	[67, 23],  [68, 5], 	[68, 32], [68, 49], 
	[69, 13], 	[69, 35], 	[70, 5], 	[70, 32], 	[71, 3],
	[71, 19], 	[72, 2], 	[72, 15], 	[72, 28], 	[73, 16],
	[74, 1], 	[74, 31], 	[74, 50], 	[75, 15], 	[76, 1],  
	[ 76, 15],	[77, 31], 	[77, 26], 	
	[78, 1],	[78, 28], 	[79, 9], 	[79, 29], 	[80, 21], 	
	[81, 8], [82, 6], 	[83, 11], 	[83, 33], 	[84, 22], 	
	[85, 14],  [87, 1], 	[88, 8], 	[89, 10], 	[90, 1],
	[91, 6], 	[92, 19], 	[95, 1], 	[96, 18], 	[98, 7]
	[100, 9], [103, 1], [106, 1], [115, 1], [109, 3], [113, 1]
...
// pour tadjwid serveur externe 
function determinePart(pageNumber) {
    // Array containing the last page number of each part
    const partEndPages = [
        22, 42, 62, 82, 102, 122, 142, 162, 182, 202,
        222, 242, 262, 282, 302, 322, 342, 362, 382, 402,
        422, 442, 462, 482, 502, 522, 542, 562, 586, 615,
    ];

    // Find the part that corresponds to the page number
    for (let i = 0; i < partEndPages.length; i++) {
        if (pageNumber <= partEndPages[i]) {
            return i + 1; // Part numbers are 1-based
        }
    }

    // If the page number exceeds the highest value, return the last part
    return partEndPages.length;
}